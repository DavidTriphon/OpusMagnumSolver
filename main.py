...
def solve_step_counts(puzzle, n=100):
    ...
    ###
    nonneg_constraints = [var >= 0 for var in vars] + \
                         [var >= 0 for var in action_vars.values()]
    ...
    variables = vars + list(action_vars.values())
    solves = find_n_solves(variables, solver, n)

    return [
        {
            str(variable): value
            for variable, value in zip(variables, solve)
...
def find_n_solves(variables, solver, n=100):
    ...
    while solver.check() == z3.sat and len(results_table) < n:
        m = solver.model()
        results = [m.evaluate(var).as_long() for var in variables]
